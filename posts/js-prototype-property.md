...
Object.getPrototypeOf显示对象o的原型为Object。
（在一些js环境下，可以通过对象的一个`__proto__`属性获取原型，不过这不是ES的规范，所以不推荐使用）
...
通过构造函数创建的对象，其内部原型链接指向的就是构造函数的prototype。

为什么通过直接量和create函数创建的对象原型不是Object，而是Object.prototype？
...
构造函数的prototype属性指向的是原型，而原型对象中的一个叫constructor的属性指向的是构造函数。
这里的关系就是说：继承关系是在对象之间进行的，但是定义继承关系的是构造函数。
如果把构造函数的原型属性改为指向其他对象，那么通过这个构造函数创建的对象也将继承自新的原型对象。

讲到这里，也就解决了第一个问题。
通过对象直接量创建的对象，它继承自Object.prototype，而Object.prototype定义了很多函数，其中就包括toString。
所以，我们创建的对象也就继承了toString函数。
...
writable定义了属性的可写性，当writable为true时，属性值才能被运算符改变。
...
更多关于属性的介绍可以点击[这里](https://developer.mozilla.org/zh-CN/docs/Web/JavaScript/Reference/Global_Objects/Object/defineProperty)

属性特性的定义可以通过object.defineProperty这个函数：
...
toString函数是Objct.prototype的属性，之所以不可以在 for/in 循环中被枚举就是因为enumerable为false。
继承Object.prototype的对象自然也继承了toString函数和其特性，所以其toString函数也不可以被枚举。